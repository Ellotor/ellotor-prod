document.addEventListener('DOMContentLoaded', function() { 
    ...
    const amountReturnedInput = document.getElementById('billing-amount-returned');
    ...
    if (submitButton) {
        submitButton.addEventListener('click', function(event) {
            ...
            const finalPaymentMode = document.querySelector('input[name="payment-mode"]:checked')?.value;
            ...
            .then(updatedData => {
                ...
                const bankingMobile = document.getElementById('Banking-mobile').value;
                const amountReturned = document.getElementById('billing-amount-returned').value;
    ...
    document.getElementById('back-button-search').addEventListener('click', function() {
        console.log('Hi');
        window.history.back();
    });
    ...
    document.getElementById('search-form').addEventListener('submit', function(event) {
        ...
        fetch(`/getDataByTokenOrMobile?mobile=${encodeURIComponent(mobile)}&stand=${encodeURIComponent(stand)}`)
            .then(response => {
                if (!response.ok) {
                    if (response.status === 404) {
                        throw new Error('User not found');
            ...
            .then(data => {
                ...
                if (data && data.length > 0) {
                    ...
                    filteredData = data.filter(record => (!record.endTime || record.endTime.trim() === ''));
                    filteredData.forEach(record => {
                        const row = document.createElement('tr');
                        // Ensure mobile number starts with '91' in the table
                        const formattedMobile = record.mobile.startsWith('91') ? record.mobile : '91' + record.mobile;
                        row.innerHTML = `
                    ...
                    document.getElementById('back-button-table').addEventListener('click', function() {
                        console.log('hello');
                        const dataDisplay = document.getElementById('data-display');
                        const searchForm = document.getElementById('search-form');
